...
class HtmlGenerator {
    ...
    extractTodoFromToolResult(data) {
        try {
            ...
            if (Array.isArray(content)) {
                for (const item of content) {
                    if (item.type === 'tool_result' && item.content) {
                        // Try to extract and parse JSON array more carefully
                        const todos = this.extractJsonArray(item.content);
                        if (todos) {
                            return this.formatTodos(todos);
                        }
                    }
            ...
            // Silently handle errors - don't show warnings to users
    ...
    /**
     * Extract JSON array from content string with better error handling
     */
    extractJsonArray(content) {
        ...
        try {
            // First try direct parsing if content is pure JSON
            return JSON.parse(content);
        } catch (error) {
            // If direct parsing fails, try to find a JSON array
                    ...
                    if (char === ']') {
                        ...
                        if (depth === 0) {
                            ...
                            try {
                                ...
                                // JSON is malformed even with correct brackets
                                return null;
                            }
                        }
                    }
